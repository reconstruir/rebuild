...
class hook_inline_register_meta(value_register_meta):
...
class value_hook_inline(with_metaclass(hook_inline_register_meta, value_base)):

  result = hook_result
  
  def __init__(self, origin = None, properties = None):
    'Create a new hook inline.'
    super(value_hook_inline, self).__init__(origin, properties = properties)
  ...
  def parse(clazz, origin, value, node):
    ...
    print('PARSING: %s - %s' % (value, str(origin)))
  ...
  def resolve(clazz, values, class_name):
    check.check_value_hook_inline_seq(values)
    assert class_name == value_type.HOOK_LIST_INLINE
    result_hooks = []
    for value in values:
      check.check_value_hook_inline(value)
      result_hooks.append(value)
    result = value_hook_list_inline(values = result_hooks)
    result.remove_dups()
    return result
...
class value_hook_list_inline(value_list_base):
  ...
  __value_type__ = value_hook_inline
  
  def __init__(self, origin = None, value = None):
    super(value_hook_list_inline, self).__init__(origin = origin, value = value)
