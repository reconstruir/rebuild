...
from .native_package_manager import native_package_manager

class native_package_manager_cli_command(object):
  'native_package_manager cli commands.'

  @classmethod
  def handle_command(clazz, command, **kargs):
    func = getattr(native_package_manager_cli_command, command)
    npm = native_package_manager()
    return func(npm, **kargs)
  
  @classmethod
  def list(clazz, npm):
    check.check_native_package_manager(npm)
  ...
  def contents(clazz, npm, package_name, levels, files_only, dirs_only):
    ...
    if files_only and dirs_only:
      raise native_package_manager_error('Only one of --files or --dirs can be given.')
    if files_only:
