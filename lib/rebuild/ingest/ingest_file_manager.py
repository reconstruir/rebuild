...
class _project_entry(namedtuple('_project_entry', 'name, filename, checksum, ingest_file')):

  def __new__(clazz, name, filename, checksum, ingest_file):
    check.check_string(name)
    check.check_string(filename)
    check.check_ingest_file(ingest_file)
    return clazz.__bases__[0].__new__(clazz, name, filename, checksum, ingest_file)
...
class ingest_file_manager(object):

  def __init__(self, checksum_getter = None):
    log.add_logging(self, 'ingest_file_manager')
...
#      raise RuntimeError('Already loaded: %s' % (filename))
    ...
    parser = ingest_file_parser(filename, text)
    ingest_files = parser.parse()
    ...
    self._filename_map[filename] = ingest_files
    self.blurb_verbose('loaded project file %s' % (path.relpath(filename)))
    
  def load_ingest_files_from_env(self):
    from_env = self.find_env_ingest_files()
    for f in from_env:
      self.log_d('loading from env: %s' % (f))
      self.load_ingest_file(f)
        
  def print_projects(self):
    for name in sorted(self._projects.keys()):
      entry = self._projects[name]
      print('%s - %s\n%s\n' % (entry.name, entry.filename, str(entry.ingest_file)))
  ...
  def resolve_ingest_files(self, filename, build_target):
    'Return the complete list of ingest_files including imports for a given project filename.'
    check.check_string(filename)
    check.check_build_target(build_target)
    filename = path.abspath(filename)
    ingest_files = self._filename_map.get(filename, None)
    if not ingest_files:
      raise RuntimeError('Filename not loaded: %s' % (filename))
    names = [ pf.name for pf in ingest_files ]
    dep_map = self._make_dep_map(self._projects, build_target)
    resolved_names = dependency_resolver.resolve_deps(dep_map, names)
    return [ self._projects[name].ingest_file for name in resolved_names ]
  ...
  def imported_projects(self, filename, build_target):
    ...
    check.check_string(filename)
    check.check_build_target(build_target)
    pfiles = self.resolve_ingest_files(filename, build_target)
  ...
  def available_recipes_dict(self, filename, build_target):
    ...
    filename = path.abspath(filename)
    result = {}
    ingest_files = self.resolve_ingest_files(filename, build_target)
    for pf in ingest_files:
  ...
  def _make_dep_map(clazz, projects, build_target):
    ...
    for name in projects.keys():
      entry = projects[name]
      imports = entry.ingest_file.resolve_imports(build_target.system)
      dep_map[name] = set(imports)
    return dep_map

  @classmethod
  def find_env_ingest_files(clazz):
  ...
  def override_projects(self, override_filename):
    ...
    parser = ingest_file_parser(override_filename, text)
    override_ingest_files = parser.parse()
    for override_pf in override_ingest_files:
      if not override_pf.name in self._projects:
        self.log_e('Project for overriding not found: %s' % (override_pf.name))
        return False
      if not self._override_project(self._projects[override_pf.name].ingest_file, override_pf):
        return False
    return True
  ...
  def print_variables(self):
    for name in sorted(self._projects.keys()):
      pf = self._projects[name].ingest_file
      for v in pf.variables:
        print('%s %s %s' % (path.relpath(pf.filename), pf.name, str(v)))
    
check.register_class(ingest_file_manager, include_seq = False)
