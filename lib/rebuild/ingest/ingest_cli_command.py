...
class ingest_cli_command(object):
  ...
  def run(clazz, project_file, fs_config_file, system, cache_dir, dry_run, verbose):
    ...
    clazz.log.log_d('run: project_file={} fs_config_file={}'.format(project_file, fs_config_file))

    project = ingest_file_parser.parse_file(project_file)

    fs = fs_registry.load_from_config_file(fs_config_file)
    clazz.log.log_d('run: fs={}'.format(fs))

    tmp_dir = temp_file.make_temp_dir()
    
    for entry in project.entries:
      values = entry.resolve_method_values(system).to_dict()
      clazz.log.log_d('run: method={} entry={}:{}'.format(entry.method.descriptor.method(), entry.name, entry.version))
#      for key, value in values.items():
#        print('  {}: {}'.format(key, value))
      ingested_filename = values['ingested_filename']
      filename = entry.download(system, cache_dir, tmp_dir)
      clazz.log.log_d('run: uploading {} to {}'.format(filename, ingested_filename))
