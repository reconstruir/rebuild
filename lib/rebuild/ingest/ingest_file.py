...
class ingest_file(namedtuple('ingest_file', 'format_version, filename, description, variables, recipes, python_code')):
  ...
  def __new__(clazz, format_version, filename, description, variables, entries):
    ...
    return clazz.__bases__[0].__new__(clazz, format_version, filename, description,
                                      variables, entries)
  ...
  def to_string(self, depth = 0, indent = 2):
    ...
    return '{magic}\n{project}\n'.format(magic = self.MAGIC, project = sproject)
  
  def _to_node(self):
    'A convenient way to make a ingest_file string is to build a graph first.'
    root = node('root')
    if self.description:
    ...
    if self.entries:
      for entry in self.entries:
        root.children.append(entry.to_node())
        root.add_child('')
    return root
