...
class _project_entry(namedtuple('_project_entry', 'name, filename, project_file')):

  def __new__(clazz, name, filename, project_file):
    check.check_string(name)
    check.check_string(filename)
    check.check_project_file(project_file)
    return clazz.__bases__[0].__new__(clazz, name, filename, project_file)
