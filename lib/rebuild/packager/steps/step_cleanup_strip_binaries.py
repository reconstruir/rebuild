...
class step_cleanup_strip_binaries(Step):
  ...
  def execute(self, argument):
    is_release = argument.script.env.config.build_target.build_type == build_type.RELEASE
    if is_release:
      ...
      if not argument.args.get('strip_binaries', False):
        return step_result(True, None)
    if not path.isdir(argument.script.stage_dir):
      return step_result(True, None)
    binary_format = argument.script.env.config.build_target.binary_format
    if not binary_format:
      return step_result(True, 'Unknown binary format: %s' % (binary_format))
    binaries = binary_detector.find_strippable_binaries(argument.script.stage_dir, format_name = binary_format)
    for b in binaries:
      self.blurb('stripping binary: %s' % (path.relpath(b)))
      if not strip.strip(argument.script.env.config.build_target, b):
