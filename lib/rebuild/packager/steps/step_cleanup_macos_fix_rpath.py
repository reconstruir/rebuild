...
class step_cleanup_macos_fix_rpath(Step):
  ...
  def execute(self, argument):
    ...
    if not path.isdir(argument.script.stage_dir):
      return step_result(True, None)
    binaries = binary_detector.find_strippable_binaries(argument.script.stage_dir, format_name = 'macho')
    blurb_binaries = [ path.relpath(b) for b in binaries ]
    for b in binaries:
      deps = [ dep for dep in library.dependencies(b) if dep.startswith(argument.script.stage_dir) ]
      for dep in deps:
