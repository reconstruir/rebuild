...
class test_build_script(unittest.TestCase):
  ...
  def test_zlib(self):
    ...
    self.assertEqual( package_descriptor('zlib', '1.2.8-1', requirements = expected_requirements, properties = expected_properties), script.package_info )
    self.assertEqual( [], script.package_info.requirements )
    self.assertEqual( [], script.package_info.build_requirements )
  ...
  def test_libjpeg(self):
    ...
    expected_requirements = []
    expected_properties = { package_descriptor.PROPERTY_CATEGORY: Category.LIB }
    self.assertEqual( package_descriptor('libjpeg', '9a-1', requirements = expected_requirements, properties = expected_properties), script.package_info )
    self.assertEqual( [], script.package_info.requirements )
    self.assertEqual( [], script.package_info.build_requirements )
  ...
  def test_libopenjpeg(self):
    ...
    self.assertEqual( expected_package_info, script.package_info )
    self.assertEqual( expected_requirements, script.package_info.requirements )
    self.assertEqual( expected_build_requirements, script.package_info.build_requirements )
  ...
  def test_libpng(self):
    ...
    self.assertEqual( expected_requirements, script.package_info.requirements )
    self.assertEqual( expected_build_requirements, script.package_info.build_requirements )
    self.assertEqual( Category.LIB, script.package_info.properties[package_descriptor.PROPERTY_CATEGORY] )
