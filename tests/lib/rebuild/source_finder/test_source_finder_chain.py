...
from rebuild.source_finder import source_finder_chain, hard_coded_source_finder
...
class test_source_finder_chain(unit_test):
  ...
  def test_find_one_finder(self):
    ...
    chain = source_finder_chain()
    chain.add_finder(hard_coded_source_finder(tarball, 'alpha', '1.2.3'))
  ...
  def xtest_find_two_finders(self):
    ...
    chain.add_finder(hard_coded_source_finder(tarball1, 'alpha', '1.2.3'))

    tarball2 = source_dir_maker.make_tarball('zip')
    chain.add_finder(hard_coded_source_finder(tarball2, 'beta', '6.6.6'))
