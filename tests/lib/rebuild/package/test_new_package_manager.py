...
from rebuild.package import artifact_manager, package, new_package_manager
...
class test_new_package_manager(unittest.TestCase):
  ...
  def _make_test_pm(clazz):
    ...
    pm_dir = path.join(root_dir, 'new_package_manager')
    if clazz.DEBUG:
      print("\nroot_dir:\n", root_dir)
    am = clazz._make_test_artifact_manager()
    return new_package_manager(pm_dir, am)
