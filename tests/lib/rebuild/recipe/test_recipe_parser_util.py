...
class test_recipe_parser_util(unit_test):
  ...
  def test_make_key_value_key_only(self):
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key', 'string') )
    
  def test_make_key_value_key_and_value(self):
    self.assertEqual( ( 'key', 'bar' ), RPU.make_key_value(self.TEST_ORIGIN, 'key:bar', 'string') )
      
  def test_make_key_value_key_and_empty_value(self):
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key:', 'string') )
    
  def test_make_key_value_key_with_comment(self):
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key  # comment', 'string') )
    
  def test_make_key_value_key_and_value_with_comment(self):
    self.assertEqual( ( 'key', 'bar' ), RPU.make_key_value(self.TEST_ORIGIN, 'key:bar  # comment', 'string') )
  ...
  def test_make_key_value_value_string_list(self):
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key', 'string_list') )
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key:', 'string_list') )
    self.assertEqual( ( 'key', [ 'bar' ] ), RPU.make_key_value(self.TEST_ORIGIN, 'key:bar', 'string_list') )
    self.assertEqual( ( 'key', [ 'foo', 'bar' ] ), RPU.make_key_value(self.TEST_ORIGIN, 'key:foo bar', 'string_list') )

  def test_make_key_value_value_key_values(self):
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key', 'key_values') )
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key:', 'key_values') )
    self.assertEqual( ( 'key', VKV(values = KVL([ ( 'foo', '5' ) ])) ), RPU.make_key_value(self.TEST_ORIGIN, 'key:foo=5', 'key_values') )
    self.assertEqual( ( 'key', VKV(values = KVL([ ( 'foo', '5' ), ( 'bar', '7' ) ])) ),
                      RPU.make_key_value(self.TEST_ORIGIN, 'key:foo=5 bar=7', 'key_values') )
    self.assertEqual( ( 'key', VKV(values = KVL([ ( 'foo', '"5 6"' ), ( 'bar', '"7 8"' ) ])) ),
                      RPU.make_key_value(self.TEST_ORIGIN, 'key:foo="5 6" bar="7 8"', 'key_values') )

  def test_make_key_value_value_bool(self):
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key', 'bool') )
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key:', 'bool') )
    self.assertEqual( ( 'key', True ), RPU.make_key_value(self.TEST_ORIGIN, 'key:True', 'bool') )
    self.assertEqual( ( 'key', True ), RPU.make_key_value(self.TEST_ORIGIN, 'key:true', 'bool') )
    self.assertEqual( ( 'key', True ), RPU.make_key_value(self.TEST_ORIGIN, 'key:1', 'bool') )
    self.assertEqual( ( 'key', False ), RPU.make_key_value(self.TEST_ORIGIN, 'key:False', 'bool') )
    self.assertEqual( ( 'key', False ), RPU.make_key_value(self.TEST_ORIGIN, 'key:false', 'bool') )
    self.assertEqual( ( 'key', False ), RPU.make_key_value(self.TEST_ORIGIN, 'key:0', 'bool') )
    self.assertEqual( ( 'key', True ), RPU.make_key_value(self.TEST_ORIGIN, 'key: True', 'bool') )
    self.assertEqual( ( 'key', False ), RPU.make_key_value(self.TEST_ORIGIN, 'key: False', 'bool') )
    
  def test_make_key_value_value_int(self):
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key', 'int') )
    self.assertEqual( ( 'key', None ), RPU.make_key_value(self.TEST_ORIGIN, 'key:', 'int') )
    self.assertEqual( ( 'key', 1 ), RPU.make_key_value(self.TEST_ORIGIN, 'key:1', 'int') )
    self.assertEqual( ( 'key', 1 ), RPU.make_key_value(self.TEST_ORIGIN, 'key: 1', 'int') )
