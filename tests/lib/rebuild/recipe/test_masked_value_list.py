...
class test_masked_value_list(unit_test):
  ...
  @classmethod
  def _int(clazz, s):
    return V.parse_mask_and_value(VO(__file__, 1, s), s, VT.INT)
    
  @classmethod
  def _string(clazz, s):
    return V.parse_mask_and_value(VO(__file__, 1, s), s, VT.STRING)
    
  @classmethod
  def _bool(clazz, s):
    return V.parse_mask_and_value(VO(__file__, 1, s), s, VT.BOOL)
    
  @classmethod
  def _string_list(clazz, s):
    return V.parse_mask_and_value(VO(__file__, 1, s), s, VT.STRING_LIST)
    
  @classmethod
  def _key_values(clazz, s):
    return V.parse_mask_and_value(VO(__file__, 1, s), s, VT.KEY_VALUES)
