...
from bes.text import line_token

class test_caca_entry(unit_test):

  def test_parse_type(self):
    self.assertEqual( entry_type.VARIABLE, caca_entry.parse(line_token(1, 'prefix=/usr/foo')).etype )
    self.assertEqual( entry_type.PROPERTY, caca_entry.parse(line_token(1, 'Name: foo')).etype )
    self.assertEqual( entry_type.BLANK, caca_entry.parse(line_token(1, '     ')).etype )
